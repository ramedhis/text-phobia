...
const state = {
  ...
  textColor: '#f2ede2',
  bgColor: '#0b0a08',
  marginTop: 90, marginRight: 90, marginBottom: 90, marginLeft: 90,
  textAlign: 'left',
  radius: 140,
  strength: 14,
  spring: 8,
...
function layoutText(){
  ...
  paragraphs.forEach(paragraph => {
    ...
    function flushLine(isLast){
      ...
      } else if(state.textAlign === 'justify' && !isLast && lineWords.length > 1){
      ...
      lineWords.forEach((word, wi) => {
        for(const ch of word){
          ...
          letters.push({
            char: ch, docX: docX + w/2, docY: docY,
            x: box.x + docX + w/2, y: box.y + docY,
...
function step(dt){
  ...
  letters.forEach(L => {
    ...
    const visible = homeY > box.y - simBuffer && homeY < box.y + box.h + simBuffer;
    if(!visible){
    ...
    if(settled){
      ...
      if(state.tremble > 0 && L.fear > 0){
        L.vx += Math.sin(t*(6+L.speedMult*3) + L.phase) * state.tremble * 0.08 * L.fear;
        L.vy += Math.cos(t*(5+L.speedMult*3) + L.phase*1.3) * state.tremble * 0.08 * L.fear;
      }
  ...
  panicLevel = panicLevel*0.9 + Math.min(100, (totalDisp/Math.max(1,letters.length))*1.6)*0.1;
}
...
function render(){
  ...
  letters.forEach(L => {
    ...
    if(state.wobble && L.fear > 0){
      ...
      const mag = Math.min(1, Math.sqrt(L.ox*L.ox+L.oy*L.oy)/120) * L.fear;
...
canvas.addEventListener('pointerup', ()=>{ isDraggingScrollbar = false; });
canvas.addEventListener('pointerleave', ()=>{ hasPointer = false; isDraggingScrollbar = false; });
...
function bindRange(id, key, valId, fmt){
  ...
  el.addEventListener('input', ()=>{
    ...
    if(key === 'radius') document.getElementById('badgeRadius').textContent = v;
  });
}
...
document.querySelectorAll('#alignToggle .chip').forEach(chip => {
  chip.addEventListener('click', ()=>{
    document.querySelectorAll('#alignToggle .chip').forEach(c=>c.classList.remove('active'));
...
document.getElementById('showRadius').addEventListener('change', (e)=>{
  ...
  document.getElementById('radiusBadge').style.display = state.showRadius ? 'block' : 'none';
});
...
document.getElementById('randomizeCalm').addEventListener('change', (e)=>{ state.randomizeCalm = e.target.checked; });
...
document.getElementById('resetBtn').addEventListener('click', ()=>{ layoutText(); });
...
document.querySelectorAll('.chip[data-w]').forEach(chip => {
  chip.addEventListener('click', ()=>{
    document.querySelectorAll('.chip[data-w]').forEach(c=>c.classList.remove('active'));
    chip.classList.add('active');
...
gifBtn.addEventListener('click', ()=>{
  const duration = Math.min(parseFloat(document.getElementById('durationInput').value) || 5, 8);
  const fps = Math.min(parseInt(document.getElementById('fpsInput').value) || 15, 20);
  const gif = new GIF({
  ...
  statusText.textContent = 'Capturing GIF frames — move your mouse over the canvas';
  statusText.classList.add('active');
  ...
  gif.on('progress', (p)=>{ progressFill.style.width = Math.round(p*100) + '%'; });
